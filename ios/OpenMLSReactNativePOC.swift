...
#if canImport(openmls_react_native_pocFFI)
import openmls_react_native_pocFFI
#endif
...
fileprivate extension RustBuffer {
    ...
    static func from(_ ptr: UnsafeBufferPointer<UInt8>) -> RustBuffer {
        try! rustCall { ffi_openmls_react_native_poc_rustbuffer_from_bytes(ForeignBytes(bufferPointer: ptr), $0) }
    }

    // Frees the buffer in place.
    // The buffer must not be used after this is called.
    func deallocate() {
        try! rustCall { ffi_openmls_react_native_poc_rustbuffer_free(self, $0) }
    }
}
...
public func mlsCreateKeypackage(registeredUserDataJsonStr: String) -> String {
    return try!  FfiConverterString.lift(try! rustCall() {
    uniffi_openmls_react_native_poc_fn_func_mls_create_keypackage(
        FfiConverterString.lower(registeredUserDataJsonStr),$0
    )
})
}
...
public func mlsRegisterUser(userId: String) -> String {
    return try!  FfiConverterString.lift(try! rustCall() {
    uniffi_openmls_react_native_poc_fn_func_mls_register_user(
...
private var initializationResult: InitializationResult = {
    ...
    let scaffolding_contract_version = ffi_openmls_react_native_poc_uniffi_contract_version()
    ...
    if (uniffi_openmls_react_native_poc_checksum_func_mls_create_keypackage() != 16848) {
        return InitializationResult.apiChecksumMismatch
    }
